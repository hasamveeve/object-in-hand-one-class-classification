...
# videos to extract/track images are located on T2 server.

cap = cv2.VideoCapture("../VIDEOS/outNewNew.mp4")
...
while cap.isOpened():
    ...
    if len(contours) > 0:
        ...
        save_img = image_orig[y1:y1 + h1, x1:x1 + w1]
        save_img = cv2.resize(save_img, (image_x, image_y))
        cv2.putText(image, "Capturing...", (30, 60), cv2.FONT_HERSHEY_TRIPLEX, 2, (127, 255, 255))
        cv2.imwrite("gestures/nvv2" + str(pic_no) + ".png", save_img)
